...
@Entity @Table(name = "carrier_booking_requests")
@Data @Builder @NoArgsConstructor @AllArgsConstructor
public class CarrierBookingRequest {
    @Id @Column(name = "cbr_id", nullable = false, updatable = false)
    private String cbrId;

    @Column(name = "cbr_number", unique = true, nullable = false, length = 30)
    ...
    @Builder.Default
    private CbrStatus status = CbrStatus.DRAFT;
    ...
    protected void onCreate() {
        if (cbrId == null) cbrId = UUID.randomUUID().toString();
        createdAt = updatedAt = LocalDateTime.now();
        if (status == null) status = CbrStatus.DRAFT;
    }
